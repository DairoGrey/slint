...
/// component's animations map with a non-empty path; the outermost
/// declaration wins and its expression evaluates in the scope that
/// declared it.
...
pub(crate) fn store_global(global: &Rc<GlobalInstance>, member: &LocalMemberIndex, value: Value) {
    if let LocalMemberIndex::Property(idx) = member {
        let g = &global.compilation_unit.globals[global.global_idx];
        // Globals never carry an animation (an `animate` never moves onto a global).
        if let Some(native) = &global.native {
            let _ = native.as_ref().set_property(&g.properties[*idx].name, value, None);
            return;
        }
        set_maybe_animated(
...
fn with_flexbox_layout_item_info(
    ...
    let mut flex_props: Vec<Value> = Vec::with_capacity(elements.len());
    let mut repeated_indices: Vec<u32> = Vec::new();
    for el in elements {
        match el {
...
fn push_repeater_flexbox_items(
    ...
    for instance in instances {
        // Flexbox needs `FlexboxLayoutItemInfo` (constraint plus flex props);
        // the default `RepeatedItemTree::flexbox_layout_item_info` impl wraps
        // the box-layout info and default-fills the props.
...
fn layout_item_info_to_value(constraint: i_slint_core::layout::LayoutInfo) -> Value {
    let mut s = crate::api::Struct::default();
    s.set_field("constraint".to_string(), constraint.into());
    Value::Struct(s)
}
...
fn push_repeater_grid_input_data(
    ...
    // `repeater_steps` and `repeated_indices`.
    ...
    match f {
        ...
        BuiltinFunction::PathPointAt => {
                ...
                && let Some((inst, flat_idx)) = resolve_item_rc_from_ref(ctx, mr)
            {
                let item_rc =
                    i_slint_core::items::ItemRc::new(vtable::VRc::into_dyn(inst), flat_idx as u32);
                let t: f32 = eval_expression(ctx, &arguments[1]).try_into().unwrap_or_default();
                return item_rc
                    .downcast::<i_slint_core::items::Path>()
                    .unwrap()
                    .as_pin_ref()
                    .point_at(&item_rc, t)
                    .to_untyped()
...
fn call_item_member_function(ctx: &EvalContext, function: &MemberReference) -> Value {
    ...
    if let Some(window) = vtable::VRef::downcast_pin::<WindowItem>(item_rc.borrow()) {
        match prop_name.as_str() {
            ...
            "start-system-move" => {
                return Value::Bool(window.start_system_move(&adapter, &item_rc));
            }
            _ => {}
        }
    }
