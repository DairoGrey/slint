...
fn default(ty: &Type, text: &str) -> Option<ConstantExpression> {
    ...
    Some(match text.as_str() {
        ...
        number => {
            let (negative, number) =
                number.strip_prefix('-').map_or((false, number), |number| (true, number));
            let (mut value, unit) =
...
fn build(l: &mut Loader) {
    ...
    item! { RenderSurfaceItem {
        in property <int> surface-id;
        in property <int> frame-generation;
        in property <length> content-offset-x;
        in property <length> content-offset-y;
        in property <bool> enabled: true;
        in property <MouseCursor> mouse-cursor;
        out property <length> mouse-x;
        out property <length> mouse-y;
    ...
    element! {
        /// A clipped renderer-backed surface populated by the public C++ render surface registry.
        @expands_to_parent_geometry
        RenderSurfaceItem: RenderSurfaceItem
    }

    item! { OverlayTextInputItem {
        in property <bool> enabled: true;
        out property <bool> has-focus;
        in property <bool> focus-on-click: true;
        in property <InputMethodHints> input-method-hints;
        in property <string> surrounding-text;
        in property <int> cursor-offset;
        in property <int> anchor-offset;
        in property <string> preedit-text;
        in property <length> caret-x;
        in property <length> caret-y;
        in property <length> caret-width: 1px;
        in property <length> caret-height: 1px;
        in property <int> input-generation;
        in property <string> clipboard-write-text;
        in property <int> clipboard-write-generation: -1;
        out property <int> event-input-generation: -1;
        out property <int> replacement-start;
        out property <int> replacement-end;
        out property <int> preedit-selection-start: -1;
        out property <int> preedit-selection-end: -1;
        out property <int> clipboard-written-generation;
        callback key-pressed(event: KeyEvent) -> EventResult;
        callback key-released(event: KeyEvent) -> EventResult;
        callback text-input(text: string);
        callback preedit-updated(text: string);
        callback composition-committed(text: string);
        callback composition-cancelled;
        callback copy-requested;
        callback cut-requested;
        callback clipboard-written;
        callback paste-received(text: string);
    } }

    element! {
        /// Platform input-method and clipboard bridge for an externally owned text model.
        @accepts_focus @expands_to_parent_geometry
        OverlayTextInputItem: OverlayTextInputItem
    }
    ...
    item! { WindowItem {
        ...
        /// Start a native system move for this window.
        ///
        /// This must be called while processing an active pointer press. Returns `true` if the
        /// platform accepted the request. Unsupported backends return `false`.
        function start-system-move() -> bool { }
    } }
